...
def verify_aspect_size(size):
    ...
    aspect = h / w if h > w else w / h      # 종횡비
...
def color_candidate_img(image, center):
    ...
    pts = np.random.randint(-15, 15, (20, 2))           # 20개의 랜덤 좌표 생성
    pts = pts + center
    for x, y in pts:                                    # 랜덤 좌표로 평행 이동
        if 0 <= x < w and 0 <= y < h:
            _, _, fill, _ = cv2.floodFill(image, fill, (x, y), 255, dif1, dif2, flags)      # 후보 영역을 유사 컬러로 채움
...
def rotate_plate(image, rect):
    ...
    crop_img = cv2.getRectSubPix(rot_img, (w, h), center)       # 후보 영역
    crop_img = cv2.cvtColor(crop_img, cv2.COLOR_BGR2GRAY)

    return cv2.resize(crop_img, (144, 28))

def kNN_train(train_fname, nclass, nsample):     # 숫자 및 문자 학습
    size = (40, 40)                                             # 숫자 영상 크기
...
def find_objects(sub_mat):      # 숫자 및 문자 객체 검색
    ...
    if text_rois:                                   # 분리된 문자 영역 누적
        ...
        w, h = x1-x0, y1-y0
        num_rois.append((x0, y0, w, h))

    return num_rois

def classify_numbers(cells, nknn, tknn, K1, K2, object_rois):       # 검출 객체 영상의 숫자 및 문자 인식
    if len(cells) != 7:
        global image
        put_string(image, "No registration plate", (0, 20), '', (0, 230, 0))
        return

    texts = "가나다라마거너더러머버서어저고노도로모보소오조구누두루무부수우주아바사자바하허호"
    ...
    _, resp1, _, _ = nknn.findNearest(datas, K1)        # 숫자 k-NN 분류
    _, [[resp2]], _, _ = tknn.findNearest(text, K2)     # 문자 k-NN 분류
...
def put_string(frame, text, pt, value=None, color=(120, 200, 90)):      # 문자열 출력 함수
...
car_no, max_no, cnt = 0, 3, 1

while True:
    image, morph = preprocessing(car_no)
    if image is None:
        if car_no < 0: car_no = max_no
        elif car_no >= max_no: car_no = 0
        continue
    ...
    rows = np.reshape(candidate_imgs, (len(candidate_imgs), -1))    # 1행 데이터로 변환
    _, results = svm.predict(rows.astype("float32"))                # 분류 수행
    result = np.where(results == 1)[0]                              # 결과가 1인 값의 위치

    plate_no = result[0] if len(result) > 0 else -1

    nknn = kNN_train("images/data/train_numbers.png", 10, 20)   # 숫자 학습
    tknn = kNN_train("images/data/train_texts.png", 40, 20)     # 문자 학습

    if plate_no >= 0:
        plate_img = preprocessing_plate(candidate_imgs[plate_no])
        cells_roi = find_objects(cv2.bitwise_not(plate_img))
        cells = [plate_img[y:y+h, x:x+w] for x, y, w, h in cells_roi]

        classify_numbers(cells, nknn, tknn, 10, 10, cells_roi)      # 숫자 객체 분류
    ...
    cv2.imshow("RegistrationPlateDetecter", image)

    key = cv2.waitKey(0)  # 키 이벤트 대기
    if key == 13 or key == 32:              # 엔터 키 이벤트나 스페이스 키 이벤트가 발생하면 다음 영상
        cnt = 1
        car_no = car_no + cnt
    elif key == 8:                          # 백스페이스 키 이벤트가 발생하면 이전 영상
        cnt = -1
        car_no = car_no + cnt
